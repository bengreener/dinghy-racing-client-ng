...
class RaceStartSequence {
    ...
    constructor(race, clock) {
        ...
        this._race = race;
        // this._signals = this._generateSignals(race, dinghyClasses);
        this._clock = clock;
    ...
    prepareForRaceStartSignal(time) {
        ...
        if (this._signals && this._prepareForRaceStartSignalHandlers.size > 0 && this._signals.some(signal => signal.time === signalTime)) {
            this._prepareForRaceStartSignalHandlers.forEach(callback => callback());
        }
    }

    makeRaceStartSignal(time) {
        if (this._signals && this._makeRaceStartSignalHandlers.size > 0 && this._signals.some(signal => signal.time === time)) {
    ...
    getNextRaceToStart(time) {
        return this._race.currentStartTime >= time ? this._race : null;
    }
    ...
    async getSignals() {
        if (!this._signals) {
            this._signals = await this._generateSignals(this._race);
        }
    ...
    async _generateSignals(race) {
        ...
        const warningFlag = {name: fleet.name + ' Class Flag'};
        const preparatoryFlag = {name: 'Blue Peter'};

        signals.push({meaning: 'Warning signal', time: race.currentStartTime.getTime() + classSignalOffsets.raise, soundSignal: {description: 'One'}, visualSignal: {flags: [warningFlag], flagsState: FlagState.RAISED}});
        signals.push({meaning: 'Preparatory signal', time: race.currentStartTime.getTime() + preparatorySignalOffsets.raise, soundSignal: {description: 'One'}, visualSignal: {flags: [preparatoryFlag], flagsState: FlagState.RAISED}});
        signals.push({meaning: preparatorySignalLowerMeaning, time: race.currentStartTime.getTime() + preparatorySignalOffsets.lower, soundSignal: preparatorySignalLowerSoundSignal, visualSignal: {flags: [preparatoryFlag], flagsState: FlagState.LOWERED}});
        signals.push({meaning: 'Starting signal', time: race.currentStartTime.getTime(), soundSignal: {description: 'One'}, visualSignal: {flags: [warningFlag], flagsState: FlagState.LOWERED}});
        if (race.type === RaceType.PURSUIT) {
            // need dinghy classes for entries in races and slowest dinghy class in fleet (to get start time offset for other classes)
            const fleet = await race.getFleet();
            const allDinghyClasses = await Promise.all([race.getDinghyClassesInRace(), fleet.getDinghyClasses()]);
            // get dinghy classes of boats signed up to race and sort in PN order
            // const sortedDinghyClasses = sortArray(allDinghyClasses[0], (dinghyClass => dinghyClass.portsmouthNumber), true);
            const sortedDinghyClasses = allDinghyClasses[0].entities.sort((a, b) => {return b.portsmouthNumber - a.portsmouthNumber})
            // first flag for the race is going to be for the slowest class in the races fleet
            // const baseClass = sortArray(allDinghyClasses[1], (dinghyClass => dinghyClass.portsmouthNumber), true)[0];
            const baseClass = allDinghyClasses[1].entities.sort((a, b) => {return b.portsmouthNumber - a.portsmouthNumber})[0];
            ...
            for(let i = 1; i < sortedDinghyClasses.length; i++) {
                ...
                signals.push({meaning: sortedDinghyClasses[i].name + ' start', time: race.currentStartTime.getTime() + offset, soundSignal: {description: 'One'}});
            }
        }
        return signals;
    }
