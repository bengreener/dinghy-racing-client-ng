...
class SessionStartSequence {
    ...
    async getSignals() {
        if (!this._signals) {
            this._signals = await this._generateSignals(this._raceStartSequences);
        }
     ...
     * @returns {Promise<Array<Signal>>}
     */
    async _generateSignals(raceStartSequences) {
        ...
        const raceSignals = await Promise.all(raceStartSequences.map(rss => {return rss.getSignals()}));
        raceSignals.forEach(signals => {
            // const signals = rss.getSignals();
